...
// Плагин для различных уведомлений
...
// Удаление, очистка содержимого папок

const ftp = require('vinyl-ftp');
// Отправка файлов через Ftp

const spritesmith = require('gulp.spritesmith');
// Создание спрайтов

const zip = require('gulp-zip');
// Архивировация файлов и директорий
...
gulp.task(
  ...
  () =>
    gulp
      ...
      .pipe(sass())
      .on('error', notify.onError()) // Преобразование scss-файлов в css, вывод уведомления в случае ошибки
...
gulp.task('sprite', () => {
  const spriteData = gulp
    .src(paths.sprites.src) // Источник изображений для спрайта
    .pipe(plumber()) // Обработка ошибок при работе плагинов обработки js-файлов
    .pipe(
      spritesmith({
...
gulp.task('zipall', () => {
  gulp
    .src(paths.zip.srcAll) // Источник файлов для архивации
...
gulp.task('zip', () => {
  gulp
    .src(paths.zip.srcProd) // Источник файлов для архивации
...
gulp.task('deploy', () => {
  ...
  const globs = paths.deploy.src; // Выгружаемые файлы и директории
  return gulp
    .src(globs, {
...
gulp.task('clean', () => del(paths.dirs.dist));
...
gulp.task('replace', () => {
  const htmlProd = gulp
    .src(paths.prod.html.src)
    .pipe(gulp.dest(paths.prod.html.dest));
  const cssProd = gulp
    .src(paths.prod.css.src)
    .pipe(gulp.dest(paths.prod.css.dest)); // Перенос css-файлов
  const jsProd = gulp
    .src(paths.prod.js.src)
    .pipe(gulp.dest(paths.prod.js.dest)); // Перенос js-файлов
  const fontsProd = gulp
    .src(paths.prod.fonts.src)
    .pipe(gulp.dest(paths.prod.fonts.dest)); // Перенос шрифтов
  const otherProd = gulp
    .src(paths.prod.other.src)
